...
export default function useClasses(props: any, baseClasses: object = {}) {
  ...
  return (className: string | string[], ...moreClasses: any[]): string => {
    ...
    if (Is.array(className)) {
      returnedClasses = (className as string[]).map((className) =>
        Obj.get(comingClasses, className, "")
      );
      returnedClasses = returnedClasses.concat(
        (className as string[]).map((className) =>
          Obj.get(baseClasses, className, "")
        )
      );
    } else {
